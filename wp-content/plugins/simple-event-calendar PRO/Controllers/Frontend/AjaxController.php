...
use GDCalendar\Helpers\Builders\YearCalendarBuilder;
    ...
    public static function calendarFrontendEventFilter()
    {
        ...
        if (strlen($date) < 8){
            ...
            if(strlen($date) === 4){
                $year = absint($date);
                $month = absint(date('m'));
                $filter_event_by_year = new YearCalendarBuilder($month,$year,$id);
                $filter_event_by_year->getCalendarYear();
            }
            /* month view */
            else{
    ...
    public static function calendarFrontendViewByType(){
        ...
        switch ($type){
            ...
            case 'week':
                ...
                break;
            case 'year':
	            if(isset($cookies['year']) && !empty($cookies['year'])){
	            	$month = 1; // Default
		            $year = absint($cookies['year']);
	            }
                $view_type = new YearCalendarBuilder($month,$year,$id);
                $view_type->getCalendarYear();
                break;
        }
        wp_die();
    }
    ...
    public static function calendarFrontendLoadView(){
	    ...
	    switch ($type){
		    ...
		    case 'year':
		    	$month = 1; //default
			    $year = $date_format;
			    $onload = new YearCalendarBuilder($month,$year,$id);
			    $onload->getCalendarYear();
			    break;
	    }
